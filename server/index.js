...
if (!process.env.JWT_SECRET) {
  throw new Error('JWT_SECRET must be configured in Railway environment variables; no fallback secret is permitted.');
}
...
function publicUser(user) {
  return {
    ...
    openrouter_key_set: Boolean(user.openrouter_key),
    huggingface_key_set: Boolean(user.huggingface_key),
...
app.put('/api/user/keys', authenticateToken, async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE id = $3
      RETURNING username, openrouter_key, huggingface_key, streak
    `;
...
app.get('/api/user/preferences', authenticateToken, async (req, res) => {
  try {
    ...
    res.json(result.rows[0] ? result.rows[0].preferences : {});
  } catch (err) {
...
app.put('/api/user/preferences', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query('UPDATE users SET preferences = $1::jsonb WHERE id = $2 RETURNING preferences', [JSON.stringify(preferences), req.user.id]);
    res.json(result.rows[0] ? result.rows[0].preferences : {});
  } catch (err) {
